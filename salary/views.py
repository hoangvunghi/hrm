...
def list_salary(request):
    ...
    total_job = SalaryHistory.objects.count()
...
def create_salary(request):
    ...
    required_fields = ['SalFrom','SalAmount','EmpID']
    ...
    if serializer.is_valid():
        position_id = request.data.get('EmpID', None)

        if SalaryHistory.objects.filter(EmpID=position_id).exists():
            return Response({"error": "Salary with this EmpID already exists",
